...
exports.default = (db, config, user) => {
    const app = express();
    ...
    app.get('/config', async (req, res) => {
        try {
            const { mysql, auth, sessionSecret } = config, safeConfig = __rest(config, ["mysql", "auth", "sessionSecret"]);
            res.send({
    ...
    app.get("/customComponent", async (req, res) => {
        try {
            ...
            const bundle = await rollup.rollup({
                input: path.join(__dirname, "../../", req.query.componentPath),
                plugins: [
    ...
    app.post('/foreignSelectRecords', async (req, res, next) => {
        ...
        const column = [
            ...view.columns,
            ...(((_c = (_b = (_a = view.recordViewPanels) === null || _a === void 0 ? void 0 : _a.map(x => { var _a; return (_a = x === null || x === void 0 ? void 0 : x.createConfig) === null || _a === void 0 ? void 0 : _a.columns; })) === null || _b === void 0 ? void 0 : _b.filter(x => x)) === null || _c === void 0 ? void 0 : _c.flat()) || [])
    ...
    app.get('/record', async function (req, res) {
        const view = config.views[req.query.viewIndex];
    ...
    app.post('/updateRecord', async function (req, res, next) {
        ...
        if (view.editCallback) {
            try {
                await axios_1.default.post(view.editCallback, {
    ...
    app.post('/deleteRecord', async function (req, res) {
        ...
        if (view.deleteCallback) {
            try {
                await axios_1.default.post(view.deleteCallback, {
                    id: req.body.primaryValue
                });
            }
            catch (e) { }
    ...
    app.post('/deleteRecordFromPanel', async function (req, res) {
        ...
        if (panel.deleteCallback) {
            try {
                await axios_1.default.post(panel.deleteCallback, {
                    id: req.body.primaryValue
                });
            }
            catch (e) { }
    ...
    app.post('/createRecord', async function (req, res, next) {
        ...
        try {
            ...
            const primaryColumn = (_b = (_a = (view || panel)) === null || _a === void 0 ? void 0 : _a.columns) === null || _b === void 0 ? void 0 : _b.find(x => x.name === (view || panel).primaryId);
            if ((primaryColumn === null || primaryColumn === void 0 ? void 0 : primaryColumn.dataType) == "uuid") {
                req.body.data[primaryColumn.name] = (0, uuid_1.v4)();
            }
            ...
            for (let key in createPayload) {
                if (((_c = columns === null || columns === void 0 ? void 0 : columns.find(x => x.name === key)) === null || _c === void 0 ? void 0 : _c.includedInCreatePayload) === false) {
            ...
            if (panel ? panel.createCallback : view.createCallback) {
                try {
                    await axios_1.default.post(panel ? panel.createCallback : view.createCallback, {
                        id: r.insertId,
                        data: req.body.data
                    });
                }
    ...
    app.get('/customQuery', async function (req, res, next) {
        const query = req.query.query.replace(/\?/g, req.query.primaryValue);
        const { err, rows } = await db.query(query);
        if (err) {
            return res.status(500).send(err);
        }
        if (req.query.viewIndex && req.query.panelIndex) {
            const panel = config.views[req.query.viewIndex].recordViewPanels[req.query.panelIndex];
    ...
    function uploadFile(column, fileParamName, req, res) {
        if (column.s3Config) {
            ...
            const s3 = new aws_sdk_1.S3({
                endpoint: new aws_sdk_1.Endpoint(s3Config.host),
            ...
            form.parse(req, function (err, fields, files) {
                ...
                if (["png", "jpeg", "jpg", "gif"].includes(extension)) {
                    const dimensions = (0, image_size_1.default)(files[fileParamName][0].path);
                ...
                if (s3Config.signedUrlNeeded) {
                    ...
                    axios_1.default.put(signedUrl, fs.readFileSync(files[fileParamName][0].path), {
            ...
            const form = new multiparty.Form({
                uploadDir: column.publicPath + column.subdirectory
            });
            form.parse(req, function (err, fields, files) {
                if (err) {
                    return res.status(500).send(err);
    ...
    app.post('/uploadFileFromBetterRichTextInput', async function (req, res, next) {
        const fileUploadProperties = JSON.parse(req.headers['file-upload-properties']);
...
async function enrichConfig(config, db) {
    var _a, _b;
    const { rows: info } = await db.query("SELECT * FROM information_schema.columns WHERE table_schema = ?", [((_a = config.mysql) === null || _a === void 0 ? void 0 : _a.database) || ((_b = config.postgresql) === null || _b === void 0 ? void 0 : _b.database)]);
...
function enrichColumn(column, tableName, info) {
    ...
    if ((infoEl === null || infoEl === void 0 ? void 0 : infoEl.IS_NULLABLE) == "NO" && (infoEl === null || infoEl === void 0 ? void 0 : infoEl.DATA_TYPE) != 'text' && (column.required !== true && column.required !== false)) {
